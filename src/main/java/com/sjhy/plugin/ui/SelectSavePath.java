...
public class SelectSavePath extends DialogWrapper {
    ...
    private boolean onOK() {
        ...
        String aliasName = nameField.getText();
        if (StringUtils.isEmpty(aliasName)) {
            Messages.showWarningDialog("请输入别名！", GlobalDict.TITLE_INFO);
            return false;
        }
        ...
        if (!matcher.find()) {
            Messages.showWarningDialog("别名需符合Java变量名规则！", GlobalDict.TITLE_INFO);
            return false;
        }
        // 针对Linux系统路径做处理
        ...
        tableInfo.setAliasName(nameField.getText());
